...
namespace LockOnStudioPlugin
{
    public class LockOnStudioBehaviour : MonoBehaviour
    {
        ...
        private float targetSize = 25.0f;
        ...
        private void Start()
        {
            ...
            boneList = ReadBoneFile("\\Plugins\\LockOnPlugin\\quickbones.txt").ToArray();
        ...
        public void LockOn(GameObject bone)
        {
            StudioChara character = instance.CurrentChara;
            if(character != null)
            {
        ...
        private void UpdateCharaBones(StudioChara character)
        {
            ...
            if(character != null)
            {
                ...
                {
                    GameObject bone = body.objBone.transform.FindLoop(prefix + boneName);
                    if(bone)
                    {
                        charaBones.Add(bone);
                    }
        ...
        private void UpdateCharaIntersections(StudioChara character)
        {
            ...
            if(character != null && character.sexType == 1)
            {
                ...
                {
                    GameObject point1 = body.objBone.transform.FindLoop(prefix + data[1]);
                    GameObject point2 = body.objBone.transform.FindLoop(prefix + data[2]);
                    if(point1 && point2)
                    {
        ...
        public void ToggleLockOnTargets()
        {
